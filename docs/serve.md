...
- **Sessions** (landing) — a case strip (sessions, agents, events, critical + high findings, attack chains, contradicted events, findings reviewed) and one card per session, worst first: product, the first prompt, start and duration, prompts / tool calls / agents / spawns / files / destinations / MCP servers, a corroboration bar (CONTRADICTED · CORROBORATED · OBSERVED · REPORTED · UNKNOWN), worst severity, attack-chain titles, tags. Add a tag or a note on the card. Click the card: the timeline opens filtered to that session with the same metadata as a header.
